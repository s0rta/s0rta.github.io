...
function preload() {
  spores = loadStrings("https://yet.earth/spores.txt");
}
  ...
  for (i = 0; i < spores.length; i++) {
    if (isWebpage(spores[i])) {
      if (isSpore(spores[i])) {
        ...
        my_spores.innerHTML +=
          "<p><a href='" + spores[i] + "'>" + spores[i] + "</a></p>";
      }
    } else {
  ...
  if (iteration == 0) {
    for (i = 0; i < spores.length; i++) {
      if (isSpore(spores[i])) {
        ...
        // loadStrings(`https://${spores[i].split("/")[2]}/spores.json`, mingle);
      }
    }
    iteration++;
  } else {
  ...
  for (i = 0; i < txt.length; i++) {
    ...
    if (unique) {
      ...
      if (isWebpage(txt[i])) {
        if (isSpore(txt[i])) {
          my_hyphae.innerHTML +=
            "<p><b><i><a href='" + txt[i] + "'>" + txt[i] + "</a></i></b></p>";
        } else {
          my_hyphae.innerHTML +=
            "<p><a href='" + txt[i] + "'>" + txt[i] + "</a></p>";
        }
      } else {
